...
disp('Bellhop - Pekeris waveguide (range dependent bottom):')
disp('TL calculation')

fid = fopen('PekerisRDB.bty','r');
s = fscanf(fid,'%s\n',[1 1]);
n = fscanf(fid,'%d\n',[1 1]);
data = fscanf(fid,'%f %f\n',[2 n]);
fclose(fid);
...
[ PlotTitle, PlotType, freq, atten, Pos, p ] = read_shd( 'PekerisRDB.shd' );
...
xlabel('Range (km)')
ylabel('Depth (m)')
title('Bellhop - Pekeris waveguide (range dependent bottom)')

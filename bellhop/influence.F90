...
CONTAINS
  SUBROUTINE InfluenceCervenyRayCen( U, epsilon, alpha, iBeamWindow2, &
    ...
    COMPLEX (KIND=_RL90) :: pVB( MaxN ), qVB( MaxN ), q, epsV( MaxN ), &
                            contri, gammaV( MaxN ), gamma, P_n, P_s
...
!!! need to add logic related to NRz_per_range
  ...
  SUBROUTINE InfluenceCervenyCart( U, epsilon, alpha, iBeamWindow2, RadiusMax )
    ...
    REAL (KIND=_RL90)    :: x( 2 ), rayt( 2 ), rayn( 2 ), Tr, Tz, zr, & 
                            Polarity = 1, c, cimag, cs, cn, csq, gradc( 2 ), & 
                            crr, crz, czz, rho, deltaz
    COMPLEX (KIND=_RL90) :: pVB( MaxN ), qVB( MaxN ), q, epsV( MaxN ), contri, &
                            gammaV( MaxN ), gamma, const
    COMPLEX (KIND=_RL90) :: tau

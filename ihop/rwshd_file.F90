...
CONTAINS
  SUBROUTINE ReadHeader( FileName, Title, Atten, PlotType )
    ...
    ! This is not used anywhere in the Fortran code for the Acoustics Toolbox, 
    ! since Matlab is used to process SHDFiles

    ! FileName is a SHDFIL for complex pressure or a GRNFIL for a Green's 
    ! function
    ! Title   arbitrary title

    INTEGER, PARAMETER                :: PRTFile = 6
! Atten : stabilizing attenuation for SCOOTER FFP runs
    ...
    CLOSE( UNIT = SHDFile )
    OPEN(  UNIT = SHDFile,   FILE = FileName, STATUS = 'OLD', &
           ACCESS = 'DIRECT', FORM = 'UNFORMATTED', RECL = 4 * LRecl )

    READ( SHDFile, REC = 1  ) LRecl, Title
    READ( SHDFile, REC = 2  ) PlotType
    READ( SHDFile, REC = 3  ) Nfreq, Pos%Ntheta, Pos%NSx, Pos%NSy, Pos%NSz, &
                              Pos%NRz, Pos%NRr, atten

    ALLOCATE( freqVec( Nfreq ), Pos%Sz( Pos%NSz ), Pos%Rz( Pos%NRz ), &
                Pos%Rr( Pos%NRr ), Pos%theta( Pos%Ntheta ), Stat = IAllocStat)
    IF ( IAllocStat /= 0 ) CALL ERROUT( 'ReadHeader', &
                                    'Too many source/receiver combinations' )

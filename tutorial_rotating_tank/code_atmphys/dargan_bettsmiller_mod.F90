...
!--------------------- interface arguments -----------------------------
   ...
   _RL , intent(in) , dimension(:,:,:) :: tin, qin, pfull, phalf
   _RL , intent(in)                    :: dt
   logical   , intent(in) , dimension(:,:):: coldT
   _RL , intent(out), dimension(:,:)   :: rain,snow, bmflag, klzbs, cape, &
       cin, invtau_bm_t, invtau_bm_q, capeflag
   _RL , intent(out), dimension(:,:,:) :: tdel, qdel, q_ref, t_ref
   integer, intent(in)                    :: bi,bj, myIter
   integer, intent(in)                    :: myThid
   _RL   , intent(in) , dimension(:,:,:), optional :: mask
   logical, intent(in) , dimension(:,:,:), optional :: conv
!-----------------------------------------------------------------------
!---------------------- local data -------------------------------------

!logical,dimension(size(tin,1),size(tin,2),size(tin,3)) :: do_adjust
   _RL,dimension(size(tin,1),size(tin,2),size(tin,3)) :: rin
   _RL,dimension(size(tin,1),size(tin,2))             :: precip, precip_t
   _RL,dimension(size(tin,3))                         :: eref, rpc
   _RL,dimension(size(tin,3))                       :: tpc
...
!    Calculate CAPE and CIN.
      ...
      _RL, intent(in), dimension(:)       :: p, phalf, tin
      _RL, intent(in), dimension(:)         :: rin
      _RL, intent(in)                       :: rdgas, rvgas, hlv, kappa, cp_air
      integer, intent(out)                   :: klzb
      _RL, intent(out), dimension(:)      :: tp
      _RL, intent(out), dimension(:)        :: rp
      _RL, intent(out)                      :: cape, cin
      integer, intent(in)                    :: i,j,bi,bj,myIter
      integer, intent(in)                    :: myThid
      integer            :: k, klcl, klfc
      logical            :: nocape
      _RL, dimension(kx)   :: tin_virtual
      _RL                :: t0
      _RL                  :: r0, es, rs, theta0, pstar, value, tlcl, &
                               a, b, dtdlnp, &
                               plcl, plzb, small
...
!#######################################################################
 ...
 _RL function mixing_ratio(vapor_pressure, pressure)

 ! calculates the mixing ratio from the vapor pressure and pressure

      implicit none
      _RL, intent(in)     :: vapor_pressure
      _RL, intent(in)   :: pressure
...
!#######################################################################
 ...
 _RL function virtual_temp(temp, r)
 ...
 ! consistent with the approximation used in the fms code
      ...
      implicit none
      _RL, intent(in)   :: temp         ! temperature
      _RL, intent(in)     :: r            ! mixing ratio

      _RL                 :: q            ! specific humidity

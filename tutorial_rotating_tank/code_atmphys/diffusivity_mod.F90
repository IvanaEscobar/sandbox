...
_RL,    intent(inout),        dimension(:,:,:) :: k_m, k_t
_RL,    intent(out),          dimension(:,:)   :: h
integer, intent (in)                            :: myThid
integer, intent(in), optional, dimension(:,:)   :: kbot

_RL, dimension(size(t,1),size(t,2),size(t,3))  :: svcp,z_full_ag, &
                                                   k_m_save, k_t_save
_RL, dimension(size(t,1),size(t,2),size(t,3)+1):: z_half_ag
_RL, dimension(size(t,1),size(t,2))            :: z_surf
...
_RL,   intent(in) ,           dimension(:,:,:) :: t, z
_RL,   intent(in) ,           dimension(:,:)   :: u_star,b_star
_RL,   intent(out),           dimension(:,:)   :: h
integer, intent(in)                             :: myThid
integer,intent(in) , optional, dimension(:,:)   :: kbot

_RL,    dimension(size(t,1),size(t,2),size(t,3))  :: rich
_RL,    dimension(size(t,1),size(t,2))            :: ws,k_t_ref
_RL,    dimension(size(t,1),size(t,2))            :: h_inner,tbot
real                                               :: rich1, rich2,&
                                                      h1,h2,svp,t1,t2
...
_RL,    intent(in)  ,           dimension(:,:,:) :: u, v
_RL,    intent(in)  ,           dimension(:,:,:) :: t, z_half
_RL,    intent(in)  ,           dimension(:,:)   :: u_star, b_star
_RL,    intent(in)  ,           dimension(:,:)   :: h
_RL,    intent(inout) ,           dimension(:,:,:) :: k_m, k_t
integer, intent (in)                                :: myThid
integer, intent(in)  , optional, dimension(:,:)   :: kbot

_RL, dimension(size(t,1),size(t,2))              :: k_m_ref, k_t_ref
_RL, dimension(size(t,1),size(t,2))              :: h_inner, factor
_RL, dimension(size(t,1),size(t,2),size(t,3)+1)  :: zm
real                                              :: h_inner_max
...
integer                                        :: k, nlev
_RL, dimension(size(z_full,1),size(z_full,2)) :: elmix, htcrit
_RL, dimension(size(z_full,1),size(z_full,2)) :: delta_u, delta_v, delta_z
...
_RL, intent(in)    , dimension(:,:,:) :: u, v
_RL, intent(in)    , dimension(:,:,:) :: t, z, zz
_RL, intent(in)    , dimension(:,:)   :: h
_RL, intent(inout) , dimension(:,:,:) :: k_m, k_t
integer, intent(in)                    :: myThid

_RL, dimension(size(t,1),size(t,2))   :: dz, b, speed2, rich, fri, &
                                          alpz, fri2
integer                                :: k
...
_RL, intent(in),    dimension (:,:,:)  ::  temp
_RL, intent(in),    dimension (:,:,:)  ::  press
_RL, intent(inout), dimension (:,:,:)  ::  k_m, k_t
integer, intent(in)                     :: myThid

      _RL, dimension (size(temp,1), size(temp,2)) :: temp_half, &
                                                      rho_half, rbop2d
      integer      :: k

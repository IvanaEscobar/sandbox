...
!type(time_type), intent(in)       :: Time
_RL, intent(in)                  :: Time
_RL, intent(in),  dimension(:,:) :: &
     net_surf_sw_down, surf_lw_down
_RL, intent(inout), dimension(:,:) :: &
     flux_t,    flux_q,     flux_r
_RL, intent(inout), dimension(:,:) :: t_surf
_RL, intent(in), dimension(:,:) :: &
...
_RL, intent(in),    dimension(:,:)  :: ocean_qflux
_RL, intent(in),    dimension(:,:)  :: mixLayDepth
_RL, intent(out),   dimension(:,:)  :: delta_t_surf
integer, intent(in) :: myThid

! ---- local variables --------------------------------------------------
_RL, allocatable, dimension(:,:)   ::                                        &
     gamma_t,               &   ! Used to calculate the implicit
     gamma_q,               &   ! correction to the diffusion in
     fn_t,                  &   ! the lowest layer
     fn_q,                  &   !

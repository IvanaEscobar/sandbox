...
_RL, intent(in)   , dimension(:) :: pt, pt0, z, z0, zt, zq, speed
_RL, intent(inout), dimension(:) :: drag_m, drag_t, drag_q
_RL, intent(inout), dimension(:) :: u_star, b_star
integer, intent(in)               :: myThid
logical, intent(in), optional, dimension(:) :: avail

_RL   , dimension(size(pt)) :: rich, fm, ft, fq, zz
logical, dimension(size(pt)) :: mask, mask_1, mask_2

_RL   , dimension(size(pt)) :: delta_b, us, bs, qs
...
_RL,    intent(in)                :: zref, zref_t
_RL,    intent(in) , dimension(:) :: u_star, b_star
_RL,    intent(in) , dimension(:) :: z, z0, zt, zq, q_star
_RL,    intent(out), dimension(:) :: del_m, del_t, del_q
integer, intent(in)                :: myThid
logical, intent(in) , optional, dimension(:) :: avail

_RL, dimension(size(z)) :: zeta, zeta_0, zeta_t, zeta_q, zeta_ref, zeta_ref_t, &
...
_RL, intent(in) , dimension(:,:,:)  :: rich
_RL, intent(out), dimension(:,:,:)  :: mix
integer, intent(in)                  :: myThid

_RL, dimension(size(rich,1),size(rich,2),size(rich,3)) :: &
      r, a, b, c, zeta, phi
...
_RL, intent(in),  dimension(:,:,:) :: z
_RL, intent(in),  dimension(:,:)   :: u_star, b_star
_RL, intent(out), dimension(:,:,:) :: k_m, k_h
integer, intent(in)                 :: myThid

_RL , dimension(size(z,1),size(z,2)) :: phi_m, phi_h, zeta, uss
integer :: j, k
INTEGER :: I
...
_RL   , intent(in) , dimension(:) :: rich, z, z0, zt, zq
logical, intent(in) , dimension(:) :: mask
_RL   , intent(out), dimension(:) :: f_m, f_t, f_q
integer, intent(in)                :: myThid

_RL, parameter    :: error    = 1.e-04
_RL, parameter    :: zeta_min = 1.e-06
integer, parameter :: max_iter = 20

_RL    :: max_cor
integer :: iter

_RL, dimension(size(rich)) ::   &
...
_RL    , intent(out),  dimension(:) :: phi_m
_RL    , intent(in),   dimension(:) :: zeta
logical , intent(in),   dimension(:) :: mask
integer, intent(in)                  :: myThid

logical, dimension(size(zeta)) :: stable, unstable
_RL   , dimension(size(zeta)) :: x
...
_RL    , intent(out),  dimension(:) :: phi_t
_RL    , intent(in),   dimension(:) :: zeta
logical , intent(in),   dimension(:) :: mask
integer, intent(in)                  :: myThid
...
_RL    , intent(out), dimension(:) :: psi_t, psi_q
_RL    , intent(in),  dimension(:) :: zeta, zeta_t, zeta_q, ln_z_zt, ln_z_zq
logical , intent(in),  dimension(:) :: mask
integer, intent(in)                 :: myThid

_RL, dimension(size(zeta)) :: x, x_t, x_q
...
_RL    , intent(out), dimension(:) :: psi_m
_RL    , intent(in),  dimension(:) :: zeta, zeta_0, ln_z_z0
logical , intent(in),  dimension(:) :: mask
integer, intent(in)                 :: myThid

_RL, dimension(size(zeta)) :: x, x_0, x1, x1_0, num, denom, y
...
_RL, intent(in)   , dimension(:,:) :: z, speed, pt, pt0, z0, zt, zq
_RL, intent(out)  , dimension(:,:) :: drag_m, drag_t, drag_q
_RL, intent(inout), dimension(:,:) :: u_star, b_star
integer, intent(in)                 :: myThid
...
_RL, intent(in)    :: z, speed, pt, pt0, z0, zt, zq
_RL, intent(out)   :: drag_m, drag_t, drag_q
_RL, intent(out)   :: u_star, b_star

_RL, dimension(1) :: pt_1, pt0_1, z_1, z0_1, zt_1, zq_1, speed_1, &
                      drag_m_1, drag_t_1, drag_q_1
_RL, dimension(1) :: u_star_1, b_star_1
integer, intent(in):: myThid
...
_RL, intent(in) , dimension(:,:) :: z, z0, zt, zq, q_star
_RL, intent(in) , dimension(:,:) :: u_star, b_star
_RL, intent(out), dimension(:,:) :: del_m, del_h, del_q
integer, intent(in)               :: myThid
...
_RL, intent(in)  :: zref, zref_t
_RL, intent(in)  :: z, z0, zt, zq, q_star
_RL, intent(in)  :: u_star, b_star
_RL, intent(out) :: del_m, del_h, del_q

_RL, dimension(1) :: z_1, z0_1, zt_1, zq_1, q_star_1, &
...
_RL   , dimension(size(z,1),size(z,2),1) :: k_m_n, k_h_n
_RL   , dimension(size(z,1),size(z,2),1) :: z_n
integer, intent(in)                       :: myThid
...
real(KIND=8), dimension(size(z),1,1) :: k_m_n, k_h_n
_RL, dimension(size(z),1,1) :: z_n
_RL, dimension(size(z),1)   :: u_star_n, b_star_n
integer, intent(in)          :: myThid
...
_RL, dimension(size(z,1),1)            :: u_star2, b_star2
_RL, dimension(size(z,1),size(z,2), 1) :: k_m2, k_h2
_RL, dimension(size(z,1),size(z,2), 1) :: z2
...
_RL, intent(in)   :: u_star, b_star
_RL, intent(in)   :: z
_RL, intent(out)  :: k_m, k_h
integer, intent(in):: myThid

_RL, dimension(1,1,1) :: k_m_n, k_h_n
_RL, dimension(1,1,1) :: z_n
_RL, dimension(1,1)   :: u_star_n, b_star_n
...
_RL, dimension(1,1)            :: u_star2, b_star2
_RL, dimension(size(z,1),1, 1) :: k_m2, k_h2
_RL, dimension(size(z,1),1, 1) :: z2
...
_RL, dimension(size(rich,1),size(rich,2),1) :: rich_3d, mix_3d
...
_RL, dimension(size(rich),1,1) :: rich_3d, mix_3d
...
_RL, dimension(1,1,1) :: rich_3d, mix_3d

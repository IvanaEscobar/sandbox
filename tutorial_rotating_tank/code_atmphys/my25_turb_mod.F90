...
!---------------------------------------------------------------------
  ...
  _RL, public, allocatable, dimension(:,:,:) :: TKE
...
!---------------------------------------------------------------------
 ...
 _RL, parameter :: aa1_old =  0.78
 _RL, parameter :: aa2_old =  0.79
 _RL, parameter :: bb1_old = 15.0
 _RL, parameter :: bb2_old =  8.0
 _RL, parameter :: ccc_old =  0.056
 _RL, parameter :: aa1_new =  0.92
 _RL, parameter :: aa2_new =  0.74
 _RL, parameter :: bb1_new = 16.0
 _RL, parameter :: bb2_new = 10.0
 _RL, parameter :: ccc_new =  0.08
 _RL, parameter :: cc1     =  0.27
 _RL, parameter :: t00     =  2.7248e2
 _RL, parameter :: small   =  1.0e-10
...
!---------------------------------------------------------------------
  _RL,    intent(in)                   :: delt
  _RL,    intent(in), dimension(:,:)   :: fracland, z0
  _RL,    intent(in), dimension(:,:,:) :: phalf, pfull, zhalf, zfull
  _RL,    intent(in), dimension(:,:,:) :: um, vm
  _RL,      intent(in), dimension(:,:,:) :: theta

  integer, intent(in), OPTIONAL, dimension(:,:)   :: kbot
  _RL,    intent(in), OPTIONAL, dimension(:,:,:) :: mask
  _RL,    intent(in), OPTIONAL, dimension(:,:)   :: ustar, bstar

!---------------------------------------------------------------------
! Arguments (Intent in/out)
!       TKE  -  turbulent kinetic energy
!---------------------------------------------------------------------
  _RL, intent(inout), dimension(:,:,:) :: TKE
...
!                         layer (m)
!---------------------------------------------------------------------
  _RL, intent(out), dimension(:,:)   :: el0
  _RL, intent(out), dimension(:,:,:) :: akm, akh
  _RL, intent(out), dimension(:,:,:) :: el
  _RL, intent(out), OPTIONAL, dimension(:,:) :: h
...
!---------------------------------------------------------------------
  ...
  _RL, dimension(SIZE(um,1),SIZE(um,2)) :: zsfc, x1, x2, akmin

  _RL, dimension(SIZE(um,1),SIZE(um,2),SIZE(um,3)-1) ::     &
        dsdzh, shear, buoync, qm2,  qm3, qm4, el2,           &
        aaa,   bbb,   ccc,    ddd,                           &
        xxm1,  xxm2,  xxm3,   xxm4, xxm5

  _RL, dimension(SIZE(um,1),SIZE(um,2),SIZE(um,3)) ::       &
        dsdz, qm,  xx1, xx2
...
!=======================================================================
  ...
  _RL,    intent(in), dimension(:,:)   :: ustar, bstar
  _RL,    intent(in), dimension(:,:)   :: zsfc
  _RL,    intent(in), dimension(:,:,:) :: zhalf, zfull
  _RL,    intent(in), dimension(:,:,:) :: akm, akh
  _RL,    intent(out),dimension(:,:)   :: h
  integer, intent(in), optional, dimension(:,:)   :: kbot

!=======================================================================
  _RL,    dimension(size(zfull,1),size(zfull,2)) :: km_surf, kh_surf
  _RL,    dimension(size(zfull,1),size(zfull,2)) :: zhalfhalf
  _RL, dimension(size(zfull,1),size(zfull,2),size(zfull,3))  :: zfull_ag
  _RL, dimension(size(zfull,1),size(zfull,2),size(zfull,3)+1):: zhalf_ag
  _RL, dimension(size(zfull,1),size(zfull,2),size(zfull,3)+1):: diff_tm
...
!                 at levels > Kbot, Mask = 0.
!---------------------------------------------------------------------
  _RL, intent(in), dimension(:,:)   :: u_star
...
!  (Intent local)
!---------------------------------------------------------------------
  _RL, dimension(SIZE(u_star,1),SIZE(u_star,2)) :: x1

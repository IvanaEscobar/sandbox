...
!---------------------------------------------------------------------
  ...
  _RL, allocatable, dimension(:) :: rhcrit, rhmax, rhmin, delrhc
...
!----------------------------------------------------------------------
  ...
  _RL, intent(in), dimension(:,:,:) :: qmix0, pfull, phalf
...
!----------------------------------------------------------------------
  _RL, intent(out), dimension(:,:,:) :: akhsc

!----------------------------------------------------------------------
! --- local
!----------------------------------------------------------------------
  _RL,    dimension(SIZE(Temp,1),SIZE(Temp,2)) ::                &
           plcl, rhumjmp, rhumscl,  xy1, xy2, xy3

  integer, dimension(SIZE(Temp,1),SIZE(Temp,2)) ::                &
           ksiglcl

  _RL,    dimension(SIZE(Temp,1),SIZE(Temp,2),SIZE(Temp,3)) ::   &
...
!---------------------------------------------------------------------
  _RL,    intent(in),  dimension(:,:)   :: tlparc, qlparc, plparc
  _RL,    intent(in),  dimension(:,:,:) :: phalf
  _RL,    intent(out), dimension(:,:)   :: plcl
...
!---------------------------------------------------------------------
  ...
  _RL,    parameter :: small    = 1.0E-2

  _RL,    dimension(size(tlparc,1),size(tlparc,2)) ::    &
           tlcl, clclo, esat, esato, desato, xy1, xy2
  _RL,  dimension(size(tlparc,1),size(tlparc,2)) ::    &
           tlclo

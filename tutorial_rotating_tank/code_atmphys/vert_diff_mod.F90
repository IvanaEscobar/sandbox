...
_RL,    intent(in)                        :: delt
_RL,    intent(in)   , dimension(:,:,:)   :: u, v, t, q,     &
                                              diff_m, diff_t, &
                                              p_half, p_full, &
                                              z_full
!real,    intent(in)   , dimension(:,:,:,:) :: tr
_RL,    intent(in)   , dimension(:,:)     :: dtau_datmos
_RL,    intent(inout), dimension(:,:)     :: tau_u, tau_v
!real,    intent(inout), dimension(:,:,:)   :: flux_tr
_RL,    intent(inout), dimension(:,:,:)   :: dt_u, dt_v, dt_t
_RL,    intent(in),    dimension(:,:,:)   :: dt_q
!real,    intent(inout), dimension(:,:,:,:) :: dt_tr
_RL,    intent(out)  , dimension(:,:,:)   :: dissipative_heat
!type(surf_diff_type), intent(inout)        :: Tri_surf
_RL,    intent(inout), dimension(:,:)     :: tri_surf_dtmass
_RL,    intent(inout), dimension(:,:)     :: tri_surf_dflux_t, tri_surf_dflux_q
_RL,    intent(inout), dimension(:,:)     :: tri_surf_delta_t, tri_surf_delta_q
_RL,    intent(out),   dimension(:,:,:)   :: e_global, f_t_global, f_q_global
integer, intent(in)                        :: myThid

integer, intent(in)   , dimension(:,:), optional :: kbot

_RL, dimension(size(u,1),size(u,2),size(u,3)) :: tt
_RL, dimension(size(u,1),size(u,2),size(u,3)) :: mu, nu

_RL, dimension(size(u,1),size(u,2)) :: f_t_delt_n1, f_q_delt_n1, &
...
_RL,    intent(in),    dimension(:,:)   :: tri_surf_delta_t, tri_surf_delta_q
_RL,    intent(in),    dimension(:,:,:) :: e_global, f_t_global, f_q_global
_RL,    intent(out),   dimension(:,:,:) :: dt_t, dt_q
integer, intent(in),    dimension(:,:), optional :: kbot
...
_RL,    intent(in)                          :: delt
_RL,    intent(in)   , dimension(:,:,:)     :: u, v, t, q, p_half, p_full, &
                                                z_full, diff_m, diff_t
_RL,    intent(in)   , dimension(:,:,:,:)   :: tr
_RL,    intent(in)   , dimension(:,:)       :: dtau_datmos, dsens_datmos, &
                                                devap_datmos
_RL,    intent(inout), dimension(:,:,:)     :: flux_tr
_RL,    intent(inout), dimension(:,:)       :: tau_u, tau_v, sens, evap
_RL,    intent(inout), dimension(:,:,:)     :: dt_u, dt_v, dt_t, dt_q
_RL,    intent(inout), dimension(:,:,:,:)   :: dt_tr
_RL,    intent(out)  , dimension(:,:,:)     :: dissipative_heat
integer, intent(in)                          :: myThid
...
_RL,    intent(in)                          :: delt
_RL,    intent(in)   , dimension(:,:,:)     :: t, q, diff, p_half, p_full, z_full
_RL,    intent(in)   , dimension(:,:,:)     :: xi
_RL,    intent(inout), dimension(:,:)       :: flux
_RL,    intent(in)   , dimension(:,:)       :: dflux_datmos
_RL,    intent(in)                          :: factor
_RL,    intent(inout), dimension(:,:,:)     :: dt_xi
integer, intent(in)                          :: myThid

integer, intent(in)   , dimension(:,:), optional :: kbot

_RL, dimension(size(xi,1),size(xi,2),size(xi,3)  ) :: mu, nu
_RL, dimension(size(xi,1),size(xi,2),size(xi,3)-1) :: e, f

_RL, dimension(size(xi,1),size(xi,2))  :: mu_delt_n, nu_n, e_n1,  &
...
_RL,    intent(in)                        :: delt
_RL,    intent(in)   , dimension(:,:,:)   :: u, v
_RL,    intent(in)   , dimension(:,:,:)   :: mu, nu
_RL,    intent(in)   , dimension(:,:)     :: dtau_datmos
_RL,    intent(inout), dimension(:,:)     :: tau_u, tau_v
_RL,    intent(inout), dimension(:,:,:)   :: dt_u, dt_v, dt_t
_RL,    intent(out)  , dimension(:,:,:)   :: dissipative_heat
...
_RL, dimension(size(u,1),size(u,2)) :: mu_delt_n, nu_n, e_n1,    &
                                        f_u_delt_n1, f_v_delt_n1, &
                                        delta_u_n, delta_v_n

_RL, dimension(size(u,1),size(u,2),size(u,3)) :: dt_u_temp, dt_v_temp

_RL, dimension(size(u,1),size(u,2),size(u,3)-1) :: e, f_u, f_v
integer :: i, j, kb

_RL    :: half_delt, cp_inv
_RL  :: one
...
!-----------------------------------------------------------------------
 ...
 call diff_surface (mu_delt_n, nu_n, e_n1, f_u_delt_n1, &
                    dtau_datmos, tau_u, one, delta_u_n, myThid)
 call diff_surface (mu_delt_n, nu_n, e_n1, f_v_delt_n1, &
                    dtau_datmos, tau_v, one, delta_v_n, myThid)
...
_RL,    intent(in)                        :: delt
_RL,    intent(in)   , dimension(:,:,:)   :: t, q, z_full
_RL,    intent(in)   , dimension(:,:,:)   :: mu, nu
_RL,    intent(in)   , dimension(:,:)     :: dsens_datmos, devap_datmos
_RL,    intent(inout), dimension(:,:)     :: sens, evap
_RL,    intent(inout), dimension(:,:,:)   :: dt_t, dt_q
integer, intent(in)                        :: myThid

integer, intent(in)   , dimension(:,:), optional :: kbot

_RL, dimension(size(t,1),size(t,2)) :: mu_delt_n, nu_n,          &
                                        e_n1, f_t_delt_n1, f_q_delt_n1, &
                                        delta_t_n, delta_q_n,      &
                                        flux1, dflux1_dsurf

_RL, dimension(size(t,1),size(t,2),size(t,3)-1) :: e, f_t, f_q
_RL, dimension(size(t,1),size(t,2),size(t,3)  ) :: tt
...
!-----------------------------------------------------------------------
 ...
 call diff_surface (mu_delt_n, nu_n, e_n1, f_q_delt_n1,  &
                    devap_datmos, evap, one, delta_q_n, myThid)
...
_RL, dimension(size(tr,1),size(tr,2)) :: mu_delt_n, nu_n, e_n1

_RL, dimension(size(tr,1),size(tr,2),size(tr,4)) :: f_delt_n1, delta_tr_n
_RL, dimension(size(tr,1),size(tr,2)) :: dflux_dtr

_RL, dimension(size(tr,1),size(tr,2),size(tr,3)-1,size(tr,4)) :: f

_RL, dimension(size(tr,1),size(tr,2),size(tr,3)-1) :: e
...
!    endif
 ...
 do n = 1, ntr
   ...
   call diff_surface (mu_delt_n, nu_n, e_n1, f_delt_n1(:,:,n),  &
                      dflux_dtr, flux(:,:,n), one, delta_tr_n(:,:,n), myThid)
...
_RL,    intent(in)    , dimension(:,:,:)   :: tr
_RL,    intent(inout) , dimension(:,:,:)   :: dt_tr
_RL,    intent(out)   , dimension(:,:,:)   :: e
_RL,    intent(out)   , dimension(:,:,:)   :: f
_RL,    intent(out)   , dimension(:,:)     :: mu_delt_n, nu_n, e_n1
_RL,    intent(out)   , dimension(:,:)     :: f_delt_n1, delta_tr_n
integer, intent(in)                         :: myThid

integer, intent(in),    dimension(:,:), optional :: kbot

_RL, dimension(size(tr,1),size(tr,2),size(tr,3)) :: a, b, c, g
...
_RL,  intent(in)                       :: delt
_RL,    intent(in)    , dimension(:,:,:) :: xi_1, xi_2
_RL,    intent(in)    , dimension(:,:,:) :: mu, nu
_RL,    intent(in)    , dimension(:,:,:) :: dt_xi_1, dt_xi_2
_RL,    intent(out)   , dimension(:,:,:) :: e, f_1, f_2
_RL,    intent(out)   , dimension(:,:)   :: mu_delt_n, nu_n, e_n1,    &
...
_RL, dimension(size(xi_1,1),size(xi_1,2),size(xi_1,3)) :: a, b, c, g, &
                                                      dt_xi_11, dt_xi_22
...
_RL, intent(in)   , dimension(:,:) :: mu_delt, nu, e_n1, f_delt_n1,  &
                                       dflux_datmos
_RL, intent(inout), dimension(:,:) :: flux, delta_xi
_RL, intent(in) :: factor
integer, intent(in)                 :: myThid

!-----------------------------------------------------------------------

 _RL, dimension(size(flux,1),size(flux,2)) :: dflux
 _RL :: fff
...
_RL,    intent(in)                      :: delt
_RL,    intent(in),    dimension(:,:,:) :: e, f
_RL,    intent(in) ,   dimension(:,:)   :: delta_xi_n
_RL,    intent(out),   dimension(:,:,:) :: dt_xi
integer, intent(in)                      :: myThid
integer, intent(in),    dimension(:,:), optional :: kbot
...
_RL,    intent(in)                       :: delt
_RL,    intent(in)    , dimension(:,:,:) :: mu, nu
_RL,    intent(out)   , dimension(:,:,:) :: e, a, b, c, g
integer, intent(in)                       :: myThid
...
!-----------------------------------------------------------------------
_RL,    intent(in)    , dimension(:,:,:) :: dt_xi, b, c, g
_RL,    intent(out)   , dimension(:,:,:) :: f
integer, intent(in)                       :: myThid
...
_RL, dimension(size(xi,1),size(xi,2),size(xi,3)) :: fluxx
...
!-----------------------------------------------------------------------
_RL,    intent(in)    , dimension(:,:,:) :: p_half
_RL,    intent(out)   , dimension(:,:,:) :: mu
integer, intent(in)                       :: myThid
...
!-----------------------------------------------------------------------
_RL,    intent(in)    , dimension(:,:,:) :: diff, p_half, p_full, &
                                             z_full, t, q
_RL,    intent(out)   , dimension(:,:,:) :: nu
integer, intent(in)                       :: myThid

_RL, dimension(size(t,1),size(t,2),size(t,3)) :: rho_half, tt
integer :: nlev
!-----------------------------------------------------------------------

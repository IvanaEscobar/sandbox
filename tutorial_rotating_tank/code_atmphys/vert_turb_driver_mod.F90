...
!type(time_type), intent(in)         :: Time, Time_next
   _RL,         intent(in)         :: Time, Time_next
   _RL,         intent(in)         :: dt
   _RL, intent(in), dimension(:,:) :: frac_land,   &
                                       u_star, b_star, rough
   _RL, intent(in), dimension(:,:,:) :: p_half, p_full, &
                                         z_half, z_full, &
                                         uu, vv, qq
!                                        u, v, t, q, um, vm, tm, qm, &
!                                        udt, vdt, tdt, qdt
   _RL, intent(in), dimension(:,:,:) :: tt
   _RL, intent(out),   dimension(:,:,:) :: diff_t, diff_m
   _RL, intent(out),   dimension(:,:)   :: gust
   integer, intent(in)                   :: myThid
   _RL, intent(in),optional, dimension(:,:,:) :: mask
integer, intent(in),optional, dimension(:,:) :: kbot
!-----------------------------------------------------------------------
_RL   , dimension(size(tt,1),size(tt,2),size(tt,3))   :: ape, thv
!logical, dimension(size(tt,1),size(tt,2),size(tt,3)+1) :: lmask
!real   , dimension(size(tt,1),size(tt,2),size(tt,3)+1) :: diag3
_RL   , dimension(size(tt,1),size(tt,2),size(tt,3)+1) :: el
_RL   , dimension(size(tt,1),size(tt,2))             :: el0, z_pbl
_RL   , dimension(size(diff_t,1),size(diff_t,2), &
